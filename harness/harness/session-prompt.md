...
Use for Phase 2–3 implementation (one logical unit per chat).
...
3. Read specs/SPECS.md — pick the highest-priority unchecked logical unit (not necessarily top-to-bottom order).
...
12. On feat/* PR units: bash scripts/auto-push.sh --dry-run, then bash scripts/auto-push.sh (use --branch for parallel workers).
13. If push fails (push/gh blocked): handoff must include "Human: bash scripts/auto-push.sh --branch feat/...".
